    ...
    try:
        ...
        for attempt in range(10):  # 최대 100초 대기
            try:
                ...
                LOG.warning(f"[네트워크] 서버 {server_id} Internal IP 조회 실패 (시도 {attempt + 1}/10): {e}")
            await asyncio.sleep(10)
        ...
        try:
            ...
            response = await _call_portforward_api(portforward_api_url, headers, ssh_payload)
...
async def create_instance(
    ...
    background_tasks.add_task(_provision_instance, session, profile, instance, request_id)
...
async def _provision_instance(session, profile, instance: InstanceCreate, request_id: str):
    try:
        ...
        try:
            ...
            lc_enabled = (
                lc_enabled_row.value
                if lc_enabled_row
                else _CONF.setting.instance_lifecycle_enabled
            )
            ...
            if lc_enabled and not is_admin_project:
                ...
                lifetime_days = int(
                    lifetime_row.value if lifetime_row else _CONF.setting.instance_lifetime_days
                )
                ...
                LOG.info(
                    f"[Lifecycle] 라이프사이클 레코드 생성: instance={server.id}, lifetime={lifetime_days}일"
                )
...
def get_port_forwarding_stats(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get port forwarding stats")
...
def add_port_forwarding(
    ...
    session = utils.generate_session(profile)
    try:
        all_fips = neutron.list_floatingips(session, profile, tenant_id=profile.project.id)[
            "floatingips"
        ]
        ...
        fip_owner = fip.get("project_id") or fip.get("tenant_id")
        if fip_owner and fip_owner != profile.project.id:
            raise HTTPException(
                status_code=403,
                detail="Floating IP does not belong to this project",
            )
        ...
        if pf_request.internal_ip not in project_internal_ips:
            raise HTTPException(
                status_code=403,
                detail="Internal IP does not belong to this project",
            )
        ...
        raise HTTPException(status_code=500, detail="Failed to create port forwarding")
...
def delete_port_forwarding(
    ...
    session = utils.generate_session(profile)
    try:
        # neutron.delete_port_forwarding_rule performs the delete with an
        # elevated system session (needed since the floating IP can live
        # outside the caller's project), so ownership must be verified here
        # first or any project could delete another project's rule.
        nc = utils.neutron_client(session=session, region=profile.region)
        try:
            fip = nc.show_floatingip(pf_delete.floating_ip_id)["floatingip"]
        except Exception:
        ...
        raise HTTPException(status_code=500, detail="Failed to delete port forwarding")
...
def list_instance_port_forwardings(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to list port forwardings")
...
async def delete_instance(
    ...
    try:
        # Verify project ownership before any privileged cleanup is attempted.
        try:
            target_server = nova.get_server(session, profile, instance_id)
        except Exception:
            raise HTTPException(status_code=404, detail="Instance not found")
        owner_id = getattr(target_server, "tenant_id", None) or getattr(
            target_server, "project_id", None
        )
        if owner_id and owner_id != profile.project.id:
            raise HTTPException(
                status_code=403,
                detail="Instance does not belong to this project",
            )
...
async def _delete_volumes_after_instance(session, profile, instance_id: str, volume_ids: list):
...
def attach_volume(
    ...
    try:
        ...
        LOG.error(
            f"Failed to attach volume {volume_action.volume_id} to instance {instance_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def detach_volume(
    ...
    try:
        ...
        LOG.error(
            f"Failed to detach volume {volume_action.volume_id} from instance {instance_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
